...
def load_term_deposits():
    ...
    onehot_encoder.fit(df[onehot_features])
    power_transformer.fit(df[numeric_features])
...
def load_credit_score():
    ...
    onehot_encoder.fit(df[onehot_features])
    elements_encoder.fit(df[element_features])
    power_transformer.fit(df[numeric_features])
    ...
    return transform(df), df["Credit_Score"]
